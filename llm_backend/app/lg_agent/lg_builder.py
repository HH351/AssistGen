...
async def analyze_and_route_query(
    ...
    # 使用结构化输出，输出问题类型
    """
    cast(TargetType, value)  类型检查，仅用于静态类型提示，运行时不改变对象
    Router: TypeDict 定义了路由器的输出结构，包含查询类型和逻辑
    model.with_structured_output(Router): 配置模型以结构化输出Router类型
    await model.with_structured_output(Router).ainvoke(messages): 异步调用模型，传入消息并获取结构化输出
    结果被强制转换为Router类型
    """
    response = cast(
...
async def respond_to_general_query(
    ...
    system_prompt = GENERAL_QUERY_SYSTEM_PROMPT.format(
        logic=state.router["logic"] #左边的logic是提示模板中的占位符，右边的是当前状态中的路由器逻辑
    )
...
builder.add_conditional_edges("analyze_and_route_query", #从这个节点触发
                              route_query  #使用这个节点判断
                              )

#框架会把节点返回的 dict 合并/更新 到全局 state 中。节点的返回值通常是一个包含部分状态字段的字典。
